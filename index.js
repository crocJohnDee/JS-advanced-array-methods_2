...
const logger = arr => arr.forEach(x => console.log(x));
...
const forEach = (arr, log) => arr.forEach(log);
...
const times3 = arr => arr.map(x => x * 3);

console.log(times3([1, 2, 3, 4, 5]));
...
// ```
const stringToNums = arr => arr.map(x => parseInt(x));

console.log(stringToNums(["4", "3", "9"]));
...
const reduceMin = arr => arr.reduce((acc, val) => (acc < val ? acc : val));

console.log(reduceMin([2, 3, 4, 5, 9]));
...
const countOddsAndEvens = arr => {
    let result = { odds: 0, evens: 0 }
    result.odds = arr.filter(x => x % 2 !== 0).length;
    result.evens = arr.filter(x => x % 2 === 0).length;
    return result
}
